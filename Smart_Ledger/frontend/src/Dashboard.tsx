...
export const Dashboard: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [txRes, summaryRes] = await Promise.all([
          fetch(`http://localhost:8000/transactions/?${queryParams.toString()}`, {
  ...
  const handleExport = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:8000/transactions/export', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/transactions/${editTx.id}`, {
        method: 'PUT',
        headers: { 
              ...
              {transactions.map(tx => (
                <tr key={tx.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    {tx.file_path ? (
                      <a 
                        href={`http://localhost:8000/${tx.file_path}`} 
                        target="_blank" 
                        rel="noreferrer"
